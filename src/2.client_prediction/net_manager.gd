...
# handles naive syncing from peer to peer. This implementation will attempt to predict the server's
# behaviour locally before overriding our client-side changes with the relay from the server's RPC
class_name ClientPrediction_NetManager
...
onready var player_scene = preload("res://2.client_prediction/player.tscn")
